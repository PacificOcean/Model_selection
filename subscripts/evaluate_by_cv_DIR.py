...
#----------データ分割関数----------
#指定変数における各値の割合を保ったまま、データをランダムにn等分する（返り値は分割毎のindex一覧が入ったリスト[2重リスト]）
def Func_divdata_labelprop(Mat_input, Sca_targetcol, Sca_groupnum, Sca_2div_ration=np.nan, Sca_seed=None): #sklearnの関数に変える
    ##########
    #Mat_input:入力データ（データフレーム）、Sca_targetcol:割合を一定に保ちたい変数名、Sca_groupnum:分割数（int）
    #Sca_2div_ration:分割数が2の時にこれが入力されると、リスト0番目の要素数がこの割合×全数になる（0<x<1のfloat）
    #Sca_seed:ランダムサンプリングのseed
    ##########

    ###型、値確認（可能性の高そうなもののみ）
    if not(isinstance(Mat_input, pd.core.frame.DataFrame)) & (isinstance(Sca_groupnum, int)) & (isinstance(Sca_2div_ration, float)): return TypeError
    if not(Mat_input.index.is_unique): return IndexError #indexは一意でなければならない
    if not(Sca_groupnum < Mat_input.shape[0]) & (1 < Sca_groupnum): return ValueError #グループ数値確認
    if not(np.isnan(Sca_2div_ration)):
        if not(Sca_2div_ration < 1) & (0 < Sca_2div_ration): return ValueError #割合値確認


    if not(Sca_seed==None): np.random.seed(Sca_seed)#seed設定

    ###return用のリスト初期化
    ret_list = []
    for i in range(0, Sca_groupnum): ret_list.append([])


    ###特定ラベル値毎に、該当レコードのindexを追加
    Vec_label = np.unique(Mat_input[Sca_targetcol].values)
    for TMP_label in Vec_label:
        TMP_Vec_index = Mat_input[Mat_input[Sca_targetcol] == TMP_label].index.values #特定ラベル値を持つindexリスト
        TMP_Vec_index = np.random.permutation(TMP_Vec_index)                          #ランダムで並び替え

        ##要素取得数設定
        if (Sca_groupnum==2) & (~np.isnan(Sca_2div_ration)):
            TMP_Sca_2div_ration = max(Sca_2div_ration, 1-Sca_2div_ration)
            TMP_Sca_divlen = int(math.ceil(1.0*len(TMP_Vec_index)*TMP_Sca_2div_ration)) #分割の要素数（要素数が多い方）
        else:
            TMP_Sca_divlen = int(math.ceil(1.0*len(TMP_Vec_index) / Sca_groupnum))  #1分割あたりの要素数

        ##要素取得
        for i ,TMP_Sca_indexhead in enumerate(range(0, len(TMP_Vec_index), TMP_Sca_divlen)):
            ret_list[i].extend(TMP_Vec_index[TMP_Sca_indexhead:TMP_Sca_indexhead+TMP_Sca_divlen])

    ##リスト順番変更（要素取得の仕様上、要素の多いグループからナンバリングされるため、割合指定時はここで調整）
    if (Sca_groupnum==2) & (~np.isnan(Sca_2div_ration)):
        if Sca_2div_ration<0.5: ret_list.reverse()


    ###リスト内リストのシャッフル（一応）
    for i in range(0, Sca_groupnum):
        ret_list[i] = list(np.random.permutation(ret_list[i]))
...
for TMP_Path_data in Vec_Path_data:
    ...
    for TMP_seed in Vec_seed:
        ...
        list_index = Func_divdata_labelprop(Mat_input=Mat_input, Sca_targetcol=Sca_flagcol, Sca_groupnum=devide_num, Sca_2div_ration=Sca_2div_ration, Sca_seed=TMP_seed)
        ...
        for tmp_lst_ix in list_index:
            ...
            train_lst_ix = list(all_index.difference(tmp_lst_ix))
            Mat_train = Mat_input.ix[train_lst_ix]
            ...
            Mat_test = Mat_input.ix[tmp_lst_ix]
