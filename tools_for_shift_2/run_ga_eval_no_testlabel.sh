...
#テストデータ
python gene_ga_data_no_testlabel.py $train_data $test_data $out_file03 $index $objective
if [ $? != 0 ];then
	echo "gene_ga_data.py ERROR"
	exit 1
fi
...
#上記で出力したデータを使用して評価
echo "step4"
python evaluate_no_testlabel.py ${train_data_path}/${train_fn}_ga_${method}.csv ${test_data_path}/${test_fn}_ga_${method}.csv $out_file04 $index $objective $out_dir $method
if [ $? != 0 ];then
