...
# [step5]
#  output_confusionmatrix.pyで閾値を変えてフラグを付け直した時の評価値を出力
# [step6]
#  plot_confusionmatrix.pyで閾値を変えてフラグを付け直した時の評価値をプロット
#
...
echo "step5"
python output_thresh_confmat.py $out_dir/${in_fn}_ga_${method}_${method_upper}prob_mean.csv $objective $out_dir cv
if [ $? != 0 ];then
...
#プロット
echo "step6"
python plot_confmat.py $out_dir/${in_fn}_ga_${method}_${method_upper}confusion_matrix.csv $out_dir
if [ $? != 0 ];then
	echo "plot_confmat.py ERROR"
	exit 1
fi

python plot_score_dist.py $out_dir/${in_fn}_ga_${method}_${method_upper}prob_mean.csv $out_dir cv
if [ $? != 0 ];then
